...
class ShellRun:
    ...
    def _run(file_name: str):
        ...
        file_path = Path(file_name).parent
        return subprocess.run(
            ["sh", file_name], capture_output=True, text=True, cwd=file_path
        )
    ...
    def add_buttons(
        ...
        Styler.add_button(
            ...
            "Run",
            Action.get_icon(file_name),
            on_click=lambda: ShellRun.run_file(file_name, output_box),
        )
    ...
    def run_file(file_name: str, container: DeltaGenerator) -> None:
        ...
        with container.spinner(f"Running {file_name}"):
            result = ShellRun._run(file_name)
...
class EditFile:
    ...
    def edit_file(file_name: str):
        ...
        match ext:
            ...
            case "sh":
                col1, col2 = button_box.columns([0.5, 0.5])
                ShellRun.add_buttons(col2, file_name, output_box)
            case _:
                col1 = button_box
